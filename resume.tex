...
\begin{itemize}[itemsep = -1 mm, leftmargin=*]
    ...
    \item Studied operational semantics, equivalence properties (in the \textit{applied pi calculus} and \textbf{Tamarin} prover) and the \textbf{SAPIC}\vspace{-0.3mm} plugin (tool translating high level protocols to multiset rewrite rules, {\it analyzable} by Tamarin)
    \item Introduced the notion of biprocesses (\textit{semantics and translation}) and {\bf diff equivalence} in SAPIC, and worked on the\vspace{-0.3mm} soundness proof of the translation after the addition 
    \end{itemize}
\end{itemize}
\vspace{-4pt}
...
\begin{itemize}[itemsep = -1 mm, leftmargin=*]
    ...
    \begin{itemize}[itemsep = -0.6 mm, leftmargin=*]
     ...
     \item Modified the Pairs strategy on a 1 year rolling window with 12\% CAGR and 0.71 overall Sharpe and researched the\vspace{-0.3mm} intricacies involved in the strategies:  Pitroski's F-Score, Mohanram's G-Score, Accruals, PEAD and Momentum crashes
    \end{itemize}
    \vspace{-3.5pt}
